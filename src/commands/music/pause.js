...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (queue.isPaused) {
      return interaction.reply({
        embeds: [EmbedUtils.warning('Already Paused', 'Music is already paused! Use `/resume` to continue playback.')],
        ephemeral: true
      });
    }
    ...
    queue.pause();
    return interaction.reply({
      embeds: [EmbedUtils.success('Music Paused', `⏸️ Paused **${queue.currentSong?.title || 'current song'}**. Use \`/resume\` to unpause.`)]
    });
  }
};
