...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription('🗑️ Delete all channels & categories in the server (Clean Server Reset)')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addBooleanOption(opt =>
      opt
        .setName('create_general')
        .setDescription('Create a clean #general text channel after deleting? (Default: True)')
        .setRequired(false)
    ),
  ...
  async execute(interaction) {
    ...
    if (!interaction.member.permissions.has(PermissionFlagsBits.Administrator) &&
      ...
      return interaction.reply({
        embeds: [EmbedUtils.error('Permission Denied', 'You need **Administrator** or **Manage Channels** permissions to delete all channels.')],
        ephemeral: true
      });
    }
    ...
    const confirmBtn = new ButtonBuilder()
      .setCustomId('confirm_delete_all_channels')
      .setLabel('Yes, Delete All Channels!')
      .setStyle(ButtonStyle.Danger)
      .setEmoji('🗑️');

    const cancelBtn = new ButtonBuilder()
      .setCustomId('cancel_delete_all_channels')
      .setLabel('Cancel')
    ...
    const warnEmbed = new EmbedBuilder()
      ...
      .setTitle('⚠️ Confirm Deleting ALL Channels')
      .setDescription(
        `Are you sure you want to **permanently delete all channels and categories** in **${guild.name}**?\n\n` +
    ...
    collector.on('collect', async i => {
      if (i.customId === 'cancel_delete_all_channels') {
        return i.update({
          embeds: [EmbedUtils.info('Cancelled', 'Channel deletion cancelled. No channels were deleted.')],
          components: []
        });
      }
    ...
    collector.on('end', async (collected, reason) => {
      if (reason === 'time' && collected.size === 0) {
        await interaction.editReply({
          embeds: [EmbedUtils.warning('Timed Out', 'Channel deletion confirmation timed out.')],
          components: []
        }).catch(() => null);
      }
    });
