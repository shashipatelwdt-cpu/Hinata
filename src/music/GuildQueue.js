...
class GuildQueue {
  ...
  async playNext() {
    ...
    try {
      ...
      // High-performance yt-dlp streaming (auto cross-platform Windows / Linux)
      ...
      if (!audioStream) {
        const playStream = await play.stream(this.currentSong.url);
        audioStream = playStream.stream;
